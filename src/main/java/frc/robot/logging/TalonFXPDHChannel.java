...
public class TalonFXPDHChannel {
    ...
    public static void registerFD(Channel channel, StatusSignal<Current> signal) {
        if (currentSignalsRio.containsKey(channel)) {return;}
    ...
    public static void registerRio(Channel channel, StatusSignal<Current> signal) {
        if (currentSignalsCanivore.containsKey(channel)) {return;}
    ...
    public static void refresh(){
        BaseStatusSignal.refreshAll(
            currentSignalsCanivore.values().toArray(BaseStatusSignal[]::new)
        );
        BaseStatusSignal.refreshAll(
            currentSignalsRio.values().toArray(BaseStatusSignal[]::new)
        );
    }
    /**<CAN ID, PDH CHANNEL> */
    public static Map<Integer, Channel> channels = Map.of(
        11, c08_FR_Steer,
        12, c09_FR_Drive,
        13, c10_BR_Steer,
        14, c11_BR_Drive,
