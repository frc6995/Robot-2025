...
public class Autos {
  ...
  public void addAutos() {
    ...
    autos.put("2.Right 3p", () -> flexAuto(POI.STE, POI.SR3, Optional.empty(), POI.E, POI.D, POI.C, POI.F));
    // autos.put("3.CenterLeft 1p", ()->flexAuto(POI.STH, POI.SL3, Optional.empty(),
    // POI.H));
    // autos.put("4.CenterRight 1p", ()->flexAuto(POI.STG, POI.SR3,
    // Optional.empty(), POI.G));
    // autos.put("AB", ()->flexAuto(POI.STA, POI.SL1, Optional.empty(), POI.A,
    // POI.B));
    ...
    autos.put("Backside H-2Alg", () -> flexAuto(POI.STH, POI.SL3, Optional.of(
        (routine) -> {
          ...
          moveback.atTimeBeforeEnd(1.4).onTrue(m_arm.goToPosition(Arm.Positions.HIGH_ALGAE_REEF))
    ...
    // new GroundAutoCycle(Optional.empty(), POI.STA, POI.A,
    // ReefScoringOption.L3_PIV),
    // new GroundAutoCycle(Optional.empty(), POI.LP1, POI.B,
    // ReefScoringOption.L3_PIV),
    // new GroundAutoCycle(Optional.empty(), POI.LP2, POI.L2_A,
    // ReefScoringOption.L2)
    // //new GroundAutoCycle(Optional.empty(), POI.LP3, POI.L2_B,
    // ReefScoringOption.L2)
    // ));
    ...
    // new GroundAutoCycle(Optional.empty(), POI.STLW, POI.L,
    // ReefScoringOption.L4_PIV),
    // new GroundAutoCycle(Optional.empty(), POI.LP1, POI.A,
    // ReefScoringOption.L4_PIV),
    // new GroundAutoCycle(Optional.empty(), POI.LP2, POI.B,
    // ReefScoringOption.L4_PIV)
    // //new GroundAutoCycle(Optional.empty(), POI.LP3, POI.L2_B,
    // ReefScoringOption.L2)
    // ));
    // autos.put("Ground C-B-A", ()->flexGroundAuto(
    // new GroundAutoCycle(Optional.empty(), POI.STC, POI.C,
    // ReefScoringOption.L4_PIV),
    // new GroundAutoCycle(Optional.empty(), POI.LP3, POI.B,
    // ReefScoringOption.L4_PIV),
    // new GroundAutoCycle(Optional.empty(), POI.LP2, POI.A,
    // ReefScoringOption.L4_PIV)
    // //new GroundAutoCycle(Optional.empty(), POI.LP3, POI.L2_B,
    // ReefScoringOption.L2)
    // ));
    // autos.put("Ground C-B-A WALL", ()->flexGroundAuto(
    // new GroundAutoCycle(Optional.empty(), POI.STCW, POI.C,
    // ReefScoringOption.L4_PIV),
    // new GroundAutoCycle(Optional.empty(), POI.LP3, POI.B,
    // ReefScoringOption.L4_PIV),
    // new GroundAutoCycle(Optional.empty(), POI.LP2, POI.A,
    ...
    // new GroundAutoCycle(Optional.empty(), POI.LP1, POI.B,
    // ReefScoringOption.L4_PIV),
    // new GroundAutoCycle(Optional.empty(), POI.LP2, POI.L2_B,
    // ReefScoringOption.L2)
    // ));
    autos.put("Push Left Front A-B", () -> sequence(
    ...
    autos.put("Push Left Wall A-B", () -> sequence(
  ...
  private record GroundAutoCycle(Optional<Function<AutoRoutine, AutoTrajectory>> after, POI start, POI score,
      ReefScoringOption scoreArm) {
  }

  public Command flexGroundAuto(GroundAutoCycle first, GroundAutoCycle second, /*
                                                                                * Optional<Function<AutoRoutine,
                                                                                * AutoTrajectory>> after,
    ...
    // bind the prev->poi (toReef) followed by the poi->intake (toIntake), followed
    // by starting the next score;
    for (Pair<GroundAutoCycle, GroundAutoCycle> pair : scorePairs) {
      ...
      var nextToReef = intake.toChecked(pair.getSecond().score, routine)
          ...
          .get();
      var recieveCoral = RobotBase.isSimulation()
    ...
    bindScore(toReef, lastCycle.scoreArm, lastCycle.after().map((opt) -> opt.apply(routine)), Arm.Positions.CORAL_STOW,
        Optional.of(() -> true));
    return routine.cmd();
  }
    ...
    return runOnce(() -> {
      targetSup.inner = target.get();
        ...
        race(
            ...
            .andThen(deadline(
                waitSeconds(0.05).andThen(m_hand.voltage(score.outtakeVoltage).withTimeout(outtakeSeconds).asProxy()),
                m_drivebase.stop())));
  ...
  public boolean hasL1Coral() {
    return m_coralSensor.hasFrontCoral();
  }
  ...
  public ReefScoringOption selectedScoringOption() {
    ...
    if (Math.abs(pivotSideTarget.minus(currentHeading).getRadians()) < Math
        .abs(batterySideTarget.minus(currentHeading).getRadians())) {
  ...
  // default -> POI.A;
  // };
  // } else if (m_board.getLevel() == 1 ||m_board.getLevel() == 2
  // ||m_board.getLevel() == 3) {
  // return switch (m_board.getBranch()) {
  // case 0 -> POI.L2_A;
  // case 1 -> POI.L2_B;
    ...
    return sequence(
        m_arm.goToPosition(finalPosition.safeWrist()).until(
            () -> m_arm.getPosition().withinTolerance(finalPosition.safeWrist(),
                Degrees.of(10).in(Radians), Inches.of(48).in(Meters), Degrees.of(360).in(Radians))),
  ...
  // Capture<Pose2d> targetSup = new
  // Capture<Pose2d>(offsetSelectedReefPose.get());
  // return
  // parallel(
  // m_drivebase.driveToPoseSupC(targetSup).asProxy(),
  ...
  public boolean pivotSideCloserToReef() {
    ...
    var currentHeading = m_drivebase.getPoseHeading();
    var pivotSideIsCloser = (Math.abs(pivotSideTarget.minus(currentHeading).getRadians()) < Math
        .abs(batterySideTarget.minus(currentHeading).getRadians()));
  ...
  public Command coralIntakeL1() {
    return parallel(
        new ScheduleCommand(m_arm.goToPosition(Arm.Positions.GROUND_CORAL_L1)),
        new ScheduleCommand(
            sequence(
                m_hand.inL1Coral()
  ...
  public Command bargeUpAndOut() {
    return deadline(
        m_hand.inAlgae()
            .until(() -> m_arm.position.elevatorMeters() > Arm.Positions.SCORE_BARGE.elevatorMeters()
  ...
  public Command bargeUpAndOutVoltage() {
    BooleanSupplier release = () -> m_arm.position.elevatorMeters() > Arm.Positions.SCORE_BARGE.elevatorMeters()
        - Units.inchesToMeters(16);
    BooleanSupplier nohardstop = () -> m_arm.position.elevatorMeters() > Arm.Positions.SCORE_BARGE.elevatorMeters()
        - Units.inchesToMeters(10);
    return sequence(
        // prep position
        parallel(
            m_arm.goToPosition(Arm.Positions.SCORE_BARGE_PRE),
            m_hand.inAlgae()).until(
        ...
        parallel(
            ...
            m_arm.elevatorS.voltage(() -> 4)).alongWith(m_hand.inAlgae()).until(release),
        // stop
        parallel(
            m_arm.mainPivotS.goTo(Arm.Positions.SCORE_BARGE::pivotRadians),
            m_arm.wristS.goTo(Arm.Positions.SCORE_BARGE::wristRadians),
            m_arm.elevatorS.voltage(() -> 4)).alongWith(m_hand.outAlgae()).until(nohardstop),
        parallel(
            m_arm.mainPivotS.goTo(Arm.Positions.SCORE_BARGE::pivotRadians),
            m_arm.wristS.goTo(Arm.Positions.SCORE_BARGE::wristRadians),
            m_arm.elevatorS.voltage(() -> 0),
            m_hand.stop()).withTimeout(0.2),
        // retract
        parallel(
            new ScheduleCommand(m_arm.goToPosition(Arm.Positions.STOW)),
            new ScheduleCommand(m_hand.stop())));
  ...
  public Rotation2d bargeTargetHeading() {
    return (m_drivebase.getPose().getX() > AllianceFlipUtil.fieldLength / 2.0) ? Rotation2d.k180deg : Rotation2d.kZero;
  }
  ...
  public boolean atBargeLine() {
    return MathUtil.isNear(bargeTargetX(), m_drivebase.getPose().getX(), Units.inchesToMeters(2)) &&
        MathUtil.isNear(0, m_drivebase.getPoseHeading().minus(bargeTargetHeading()).getRadians(),
            Units.degreesToRadians(10));
  }

  // public Command outtake() {
  // return m_hand.outCoral().alongWith(runOnce(() ->
  // m_coralSensor.setHasCoral(false)));
  // }

  public AutoTrajectory bindScore(AutoTrajectory self, ReefScoringOption scoringPosition, Optional<AutoTrajectory> next,
      ArmPosition intakeAfterScore, Optional<BooleanSupplier> readyToMoveOn) {
    BooleanSupplier elevatorRetractedEnough = () -> m_arm.position.elevatorLength().lt(Meters.of(1.45));
    ...
    // self.atTranslation(finalPoseUnflipped.getTranslation(),
    // Units.inchesToMeters(12))
    // .onTrue(
    ...
    self.done()
        ...
        .onTrue(
            sequence(
                ...
                next.map((Function<AutoTrajectory, Command>) (nextTraj) -> waitUntil(
                    readyToNext)
                    .andThen(nextTraj.spawnCmd())).orElse(none())));
    return self;
  }

  public Command flexAuto(POI start, POI intake, Optional<Function<AutoRoutine, AutoTrajectory>> after, POI firstScore,
      POI... rest)
      throws NoSuchElementException {
    SwerveRequest stationPickupRequest = new SwerveRequest.ApplyRobotSpeeds()
        .withDriveRequestType(DriveRequestType.Velocity)
      ...
      toIntake
          ...
          .onTrue(
              sequence(
                  deadline(
                      recieveCoral,
                      m_drivebase.applyRequest(() -> stationPickupRequest)),
                  nextToReef.spawnCmd()));
      toReef = nextToReef;
    }
    bindScore(toReef, scoringPosition, after.map((func) -> func.apply(routine)), Arm.Positions.WALL_INTAKE_CORAL,
        Optional.empty());
    return routine.cmd();
  }
