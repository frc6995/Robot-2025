...
@Logged
public class Arm {
    public final MechanismLigament2d ARM;
    public Arm() {
    ...
    private static final Distance SAFE_PIVOT_ELEVATOR_LENGTH = ElevatorConstants.MIN_LENGTH.plus(Inches.of(3));
    public Command goToPosition(ArmPosition position) {
        return defer(()->{
            ...
            if (Math.abs(dPivot) < Units.degreesToRadians(1)) {
                    return goDirectlyTo(
                        position.pivotRadians(),
                        position.elevatorMeters(),
                        position.wristRadians());
            } else {
            double prePivotElevator = Math.min(startElevator, SAFE_PIVOT_ELEVATOR_LENGTH.in(Meters));
            double postPivotElevator = Math.min(position.elevatorMeters(), SAFE_PIVOT_ELEVATOR_LENGTH.in(Meters));
            return sequence(
                // Retract elevator
                goDirectlyTo(
                    startMainPivot,
                    prePivotElevator,
                    startWrist)
                .until(()-> Math.abs(elevatorS.getLengthMeters() - prePivotElevator) < Units.inchesToMeters(1)),
                goDirectlyTo(position.pivotRadians(),
                    postPivotElevator,
                    startWrist)
                .until(()-> Math.abs(elevatorS.getLengthMeters() - postPivotElevator) < Units.inchesToMeters(1)),
                goDirectlyTo(
                    position.pivotRadians(),
                    position.elevatorMeters(),
                    position.wristRadians())
    ...
    private Command goDirectlyTo(double mainPivotRadians, double elevatorMeters, double wristRadians) {
        return parallel(
            ...
            elevatorS.goToLength(()->elevatorMeters)
        );
    }
}
