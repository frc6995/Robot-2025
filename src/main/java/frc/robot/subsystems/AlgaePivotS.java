...
public class AlgaePivotS extends SubsystemBase {
  public class AlgaePivotConstants {
    ...
    public static final Per<VoltageUnit, AngularVelocityUnit> K_V = VoltsPerRadianPerSecond.ofNative(1.3684);
    public static final Per<VoltageUnit, AngularAccelerationUnit> K_A = VoltsPerRadianPerSecondSquared
        .ofNative(0.012185);

    public static final LinearSystem<N2, N1, N2> PLANT = LinearSystemId
        .identifyPositionSystem(K_V.in(VoltsPerRadianPerSecond), K_A.in(VoltsPerRadianPerSecondSquared));
    ...
    private static TalonFXConfiguration configureMotor(TalonFXConfiguration config) {
      ...
      config.Feedback
          .withSensorToMechanismRatio(MOTOR_ROTATIONS_PER_PIVOT_ROTATION);
  ...
  setupVisualizer();
    ...
    motor.getConfigurator().apply(
        AlgaePivotConstants.configureMotor(
            config));
  }
  ...
  public void simulationPeriodic() {
    ...
    var rotorPos = Units.radiansToRotations(m_pivotSim.getAngleRads())
        * AlgaePivotConstants.MOTOR_ROTATIONS_PER_PIVOT_ROTATION;
    var rotorVel = Units.radiansToRotations(m_pivotSim.getVelocityRadPerSec())
        * AlgaePivotConstants.MOTOR_ROTATIONS_PER_PIVOT_ROTATION;
