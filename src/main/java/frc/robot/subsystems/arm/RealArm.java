...
public class RealArm extends Arm {
  ...
  public Trigger elevatorRetractedEnough =
      new Trigger(
          () ->
              elevatorS.getLengthMeters()
                  < SAFE_PIVOT_ELEVATOR_LENGTH.in(Meters) + Units.inchesToMeters(10));
        ...
        either(
            ...
            sequence(
                ...
                goDirectlyTo(positionPivotRadians, postPivotElevator, positionWristRadians)
                    .until(
                        () ->
                            Math.abs(mainPivotS.getAngleRadians() - positionPivotRadians)
                                < Units.degreesToRadians(10)),
                goDirectlyTo(positionPivotRadians, positionElevatorMeters, positionWristRadians)),
            () ->
                Math.abs(positionPivotRadians - mainPivotS.getAngleRadians())
  ...
  public Command goToPosition(ArmPosition position) {
    if (position.pivotRadians() < Units.degreesToRadians(17)) {
      double shrunkenElevator =
          MathUtil.clamp(
              position.elevatorMeters(),
              MIN_ELEVATOR_LENGTH.in(Meters),
              SAFE_PIVOT_ELEVATOR_LENGTH.in(Meters));
      Angle wristSafeToLower =
          position.wristAngle().lt(Degrees.of(0)) ? Degrees.of(0) : position.wristAngle();
  ...
  private Command wristDirectlyTo(
      double wristRadians, double elevatorSetpointMeters, DoubleSupplier mainPivotRadians) {
    return wristS.goTo(
        () -> {
          double dontGoIntoBumperMinLimit =
              mainPivotRadians.getAsDouble() < Units.degreesToRadians(17)
                      || mainPivotS.getAngleRadians() < Units.degreesToRadians(17)
                  ? 0
                  : WristConstants.CW_LIMIT.in(Radians);
          // Only if nearly fully extended can the wrist go to its full limit, since the algae
          // roller touches the tube
          double dontGoIntoElevatorMaxLimit =
              Math.min(elevatorS.getLengthMeters(), elevatorSetpointMeters)
                      < ElevatorConstants.MAX_LENGTH.in(Meters) - Units.inchesToMeters(6)
  ...
  private Command goDirectlyToPivotHold(double elevatorMeters, double wristRadians) {
  ...
  public Command processorWithHome() {
    ...
    return sequence(
        goToPosition(Arm.Positions.SCORE_PROCESSOR)
            .until(
                () ->
                    this.position.withinTolerance(
                        Arm.Positions.SCORE_PROCESSOR,
                        Units.degreesToRadians(2),
                        Units.inchesToMeters(0.5),
                        Units.degreesToRadians(3600))),
        parallel(
            new ScheduleCommand(wristS.driveToHome()),
            new ScheduleCommand(elevatorDirectlyTo(Arm.Positions.SCORE_PROCESSOR.elevatorMeters())),
            new ScheduleCommand(mainPivotS.goTo(Arm.Positions.SCORE_PROCESSOR::pivotRadians))));
  }
}
