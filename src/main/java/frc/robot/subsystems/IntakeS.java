...
public class IntakeS extends SubsystemBase {
  ...
  public class HandConstants {
    ...
    public static final int ROLLER_MOTOR_CAN_ID = 51;
    public static final int RIGHT_MOTOR_CAN_ID = 53;
    public static final int LEFT_MOTOR_CAN_ID = 54;

    public static final double IN_CORAL_VOLTAGE = 8;
    public static final double GROUND_L1_CORAL_VOLTAGE = 8;
    ...
    public static TalonFXConfiguration configureRightMotor(TalonFXConfiguration config) {
      config.CurrentLimits.withStatorCurrentLimit(120).withStatorCurrentLimitEnable(true);
      config.Feedback.SensorToMechanismRatio = 16.0 / 3.0;
      // volts per wheel rotation = 8-9 inches of coral
      // config.Slot0.withKP(6);
    ...
    public static TalonFXConfiguration configureLeftMotor(TalonFXConfiguration config) {
      config.CurrentLimits.withStatorCurrentLimit(120).withStatorCurrentLimitEnable(true);
      config.Feedback.SensorToMechanismRatio = 16.0 / 3.0;
      // volts per wheel rotation = 8-9 inches of coral
      // config.Slot0.withKP(6);
  ...
  private final TalonFX rollerMotor = new TalonFX(HandConstants.ROLLER_MOTOR_CAN_ID, Robot.m_notSwerveBus);
  private final TalonFX leftMotor = new TalonFX(HandConstants.RIGHT_MOTOR_CAN_ID, Robot.m_notSwerveBus);
  private final TalonFX rightMotor = new TalonFX(HandConstants.LEFT_MOTOR_CAN_ID, Robot.m_notSwerveBus);
  ...
  public StatusSignal<Angle> m_positionSig = rollerMotor.getPosition();
  public StatusSignal<Voltage> m_voltageSig = rollerMotor.getMotorVoltage();
  ...
  public IntakeS() {
    super();
    rollerMotor.getConfigurator().apply(HandConstants.configureRollerMotor(new TalonFXConfiguration()));
    leftMotor.getConfigurator().apply(HandConstants.configureRightMotor(new TalonFXConfiguration()));
    rightMotor.getConfigurator().apply(HandConstants.configureLeftMotor(new TalonFXConfiguration()));

    // leftMotor.setControl(new Follower(rollerMotor.getDeviceID(), false));
    // rightMotor.setControl(new Follower(rollerMotor.getDeviceID(), false));
  ...
  public void simulationPeriodic() {
    var simState = rollerMotor.getSimState();
  ...
  public Command rollerVoltage(DoubleSupplier voltage) {
    return this.run(() -> rollerMotor.setControl(voltageRequest.withOutput(voltage.getAsDouble())));
  }

  public Command rollerVoltage(double voltage) {
    return rollerVoltage(() -> voltage);
  }
  ...
  public Command inCoral() {
    return rollerVoltage(HandConstants.IN_CORAL_VOLTAGE);
  }

  public Command inCoralSlow() {
    return rollerVoltage(2.5);
  }

  public Command outCoral() {
    return rollerVoltage(HandConstants.OUT_CORAL_VOLTAGE);

  }

  public Command outCoralReverse() {
    return rollerVoltage(HandConstants.OUT_CORAL_VOLTAGE_REVERSE);

  }

  public Command outCoralSlow() {
    return rollerVoltage(HandConstants.OUT_CORAL_VOLTAGE_SLOW);

  }

  public Command inAlgae() {
    return rollerVoltage(HandConstants.IN_ALGAE_VOLTAGE);
  }

  public Command outAlgae() {
    return rollerVoltage(10);// HandConstants.OUT_ALGAE_VOLTAGE);
  }

  public Command outAlgaeSlow() {
    return rollerVoltage(HandConstants.OUT_ALGAE_VOLTAGE_SLOW);
  }
