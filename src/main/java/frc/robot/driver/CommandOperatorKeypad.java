...
public class CommandOperatorKeypad {
  ...
  private IntegerPublisher selectionEntry =
      NetworkTableInstance.getDefault().getIntegerTopic("/DriverDisplay/selection").publish();
  ...
  public Trigger key(Button key) {
    return m_hid
        .button(key.value, CommandScheduler.getInstance().getDefaultButtonLoop())
        .castTo(Trigger::new);
  }
