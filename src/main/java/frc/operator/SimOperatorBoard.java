...
public class SimOperatorBoard extends OperatorBoard {
  ...
  private void branchCmd(int branch) {
    isSim
        .and(hid.button(branch + 1))
        .onTrue(Commands.runOnce(() -> selectedBranch = branch).ignoringDisable(true));
  }

  private void climbCmd(int cage) {
    isSim
        .and(hid.button(cage + 12 + 4 + 1))
        .onTrue(Commands.runOnce(() -> selectedClimb = cage).ignoringDisable(true));
  }

  private void levelCmd(int level) {
    isSim
        .and(hid.button(level + 12 + 1))
        .onTrue(Commands.runOnce(() -> selectedLevel = level).ignoringDisable(true));
  }
